...
import styles from './Bai4.module.scss';
...
function Bai4() {
    ...
    return (
        <div style={{ padding: 20 }}>
            <input className={cx('input')} value={job} type="text" onChange={(e) => setJob(e.target.value)} />
                ...
                {jobs.map((job, index) => {
                    return (
                        <div key={index} style={{ display: 'flex', gap: 10 }}>
                            <li>{job}</li>
                            <button className={cx('btn-delete-item')} onClick={() => handleRemoveItem(index)}>
